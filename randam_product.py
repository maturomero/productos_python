...
def show_list():
    if lista:
        ...
        list_window.title("Lista de artículos")
        list_text = tk.Text(list_window, wrap="word")
        list_text.insert("1.0", items)
        list_text.config(state=tk.NORMAL)  # Permitir la edición (necesario para la selección)
        list_text.pack(expand=True, fill="both", padx=10, pady=10)
        list_text.config(state=tk.DISABLED)  # Deshabilitar la edición
    else:
